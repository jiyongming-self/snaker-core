...
public abstract class AbstractDBAccess implements DBAccess {
	...
	protected static final String CALLBACK_INSERT = "insert into wf_callback_log (id,order_id,variable,result,create_time,process_name,apply_name,count,status) values (?,?,?,?,?,?,?,?,?)";

	protected static final String PROCESS_INSERT = "insert into wf_process (id,name,display_Name,type,instance_Url,state,version,create_Time,creator,callback,host_name) values (?,?,?,?,?,?,?,?,?,?,?)";
	protected static final String PROCESS_UPDATE = "update wf_process set name=?, display_Name=?,state=?,instance_Url=?,create_Time=?,creator=?,callback=?,host_name=? where id=? ";
	...
	protected static final String TASK_INSERT = "insert into wf_task (id,order_Id,task_Name,display_Name,task_Type,perform_Type,operator,create_Time,finish_Time,expire_Time,action_url_type,action_Url,parent_Task_Id,variable,version) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	protected static final String TASK_UPDATE = "update wf_task set finish_Time=?, operator=?, version = version + 1 where id=? and version = ?";
	protected static final String TASK_HISTORY_INSERT = "insert into wf_hist_task (id,order_Id,task_Name,display_Name,task_Type,perform_Type,task_State,operator,create_Time,finish_Time,expire_Time,action_url_type,action_Url,parent_Task_Id,variable) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	protected static final String TASK_DELETE = "delete from wf_task where id = ?";
	
	protected static final String TASK_ACTOR_INSERT = "insert into wf_task_actor (task_Id, actor_Id, actor_type) values (?, ?, ?)";
	protected static final String TASK_ACTOR_HISTORY_INSERT = "insert into wf_hist_task_actor (task_Id, actor_Id, actor_type, is_operator) values (?, ?, ?, ?)";
	...
	protected static final String HIST_TASK_ACTOR_DELETE = "delete from wf_hist_task_actor where task_Id = ?";
	protected static final String HIST_TASK_DELETE = "delete from wf_hist_task where id = ?";

	protected static final String QUERY_VERSION = "select max(version) from wf_process ";
	protected static final String QUERY_PROCESS = "select id,name,display_Name,type,instance_Url,state, content, version,create_Time,creator,callback,host_name from wf_process ";
	...
	protected static final String QUERY_TASK = "select id,order_Id,task_Name,display_Name,task_Type,perform_Type,operator,create_Time,finish_Time,expire_Time,action_url_type,action_Url,parent_Task_Id,variable, version from wf_task ";
	protected static final String QUERY_TASK_ACTOR = "select task_Id, actor_Id, actor_type from wf_task_actor ";
	...
	protected static final String QUERY_HIST_TASK = "select id,order_Id,task_Name,display_Name,task_Type,perform_Type,task_State,operator,create_Time,finish_Time,expire_Time,action_url_type,action_Url,parent_Task_Id,variable from wf_hist_task ";
	protected static final String QUERY_HIST_TASK_ACTOR = "select task_Id, actor_Id, actor_type, is_operator from wf_hist_task_actor ";
	...
	public void saveProcess(Process process) {
		if(isORM()) {
			...
			process.getCallback(),process.getHostName()};
	...
	public void updateProcess(Process process) {
		if(isORM()) {
			...
			Object[] args = new Object[]{process.getName(), process.getDisplayName(), process.getState(), 
					process.getInstanceUrl(), process.getCreateTime(), process.getCreator(),process.getCallback(),process.getHostName(), process.getId()};
			int[] type = new int[]{Types.VARCHAR, Types.VARCHAR, Types.INTEGER, Types.VARCHAR, Types.VARCHAR, Types.VARCHAR, Types.VARCHAR,Types.VARCHAR, Types.VARCHAR};
	...
	public void saveTask(Task task) {
		if(isORM()) {
			...
			Object[] args = new Object[]{task.getId(), task.getOrderId(), task.getTaskName(), task.getDisplayName(), task.getTaskType(), 
					...
					task.getExpireTime(), task.getActionUrlType(),task.getActionUrl(), task.getParentTaskId(), task.getVariable(), task.getVersion()};
	...
	public void saveTaskActor(TaskActor taskActor) {
        if(isORM()) {
            ...
            saveOrUpdate(buildMap(TASK_ACTOR_INSERT, new Object[]{taskActor.getTaskId(), taskActor.getActorId(), taskActor.getActorType() }, type));
        }
    }
	...
	public void saveHistory(HistoryTask task) {
		if(isORM()) {
			saveOrUpdate(buildMap(task, SAVE));
            if(task.getActorList() != null) {
                for(HistoryTaskActor hist : task.getActorList()) {
			...
			Object[] args = new Object[]{task.getId(), task.getOrderId(), task.getTaskName(), task.getDisplayName(), task.getTaskType(), 
					...
					task.getExpireTime(), task.getActionUrlType(), task.getActionUrl(), task.getParentTaskId(), task.getVariable()};
			int[] type = new int[]{Types.VARCHAR, Types.VARCHAR, Types.VARCHAR, Types.VARCHAR, Types.INTEGER, 
			...
			if(task.getActorList() != null) {
				for(HistoryTaskActor hist : task.getActorList()) {
					saveOrUpdate(buildMap(TASK_ACTOR_HISTORY_INSERT,
							new Object[]{hist.getTaskId(), hist.getActorId(), hist.getActorType(), hist.getIsOperator()},
	...
	public List<HistoryTask> getHistoryTasks(Page<HistoryTask> page, QueryFilter filter) {
		...
		if(isFetchActor) {
			sql.append("and ta.is_operator=1 ");
			sql.append(" and ta.actor_Id in (");
			for(int i = 0; i < filter.getOperators().length; i++) {
				sql.append("?,");
				paramList.add(filter.getOperators()[i]);
	...
	public List<WorkItem> getHistoryWorkItems(Page<WorkItem> page, QueryFilter filter) {
		...
		sql.append(" left join wf_process p on p.id = o.process_id ");
		sql.append(" where 1=1 and ta.is_operator=1 ");
		/**
		 * 查询条件构造sql的where条件
		 */
		List<Object> paramList = new ArrayList<Object>();
	 ...
	 * @return
	 */
	public List<WfGroupPerson> getPersonByGoupId(String groupId) {
		...
		return this.queryList(WfGroupPerson.class, sql.toString(), paramList.toArray());

	}

	@Override
	public List<String> getRealOperatorList(String orderId, List<String> taskNameList) {
		log.info("getRealOperatorList参数：orderId={},taskeNameList={}", orderId,taskNameList);
		...
		StringBuffer sql = new StringBuffer("select operator from wf_hist_task where 1=1 ");
		List<Object> paramList = new ArrayList<>();
		sql.append(" and order_id = ? ");
		...
		for(int i = 0; i < taskNameList.size(); i++) {
			sql.append("?,");
			paramList.add(taskNameList.get(i));
		}
		sql.deleteCharAt(sql.length() - 1);
		sql.append(") ");
		sql.append(" and create_time >=  ");
		sql.append("(select create_time from wf_hist_task where order_id = ? ");
		paramList.add(orderId);
		sql.append(" and parent_task_id like 'start' order by create_time desc limit 1)");
		//List<Map> maps = queryList(Map.class, sql.toString(), paramList.toArray());
		//return this.queryList(String.class, sql.toString(), paramList.toArray());
		Object o = queryCount(sql.toString(), paramList.toArray());
		if (o == null) {
			return null;
		}
		if (o instanceof String) {
			List<String> operatorList = new ArrayList<>(1);
			operatorList.add(o.toString());
			return operatorList;
		}
		return (List<String>) queryCount(sql.toString(), paramList.toArray());
	}

	public void deleteHistTask(HistoryTask hist) {
		if(!isORM()) {
			Object[] args = new Object[]{hist.getId()};
			int[] type = new int[]{Types.VARCHAR};
			saveOrUpdate(buildMap(HIST_TASK_ACTOR_DELETE, args, type));
			saveOrUpdate(buildMap(HIST_TASK_DELETE, args, type));
		}
	}
	 ...
	 * @param callbackLog
	 */
	public void saveCallbackLog(CallbackLog callbackLog){
		if(isORM()) {
			saveOrUpdate(buildMap(callbackLog, SAVE));
		} else {
			Object[] args = new Object[]{callbackLog.getId(), callbackLog.getOrderId(), callbackLog.getVariable(), callbackLog.getResult(),
					callbackLog.getCreateTime(),callbackLog.getProcessName(),callbackLog.getApplyName(),
			callbackLog.getCount(),callbackLog.getStatus()};
			int[] type = new int[]{Types.VARCHAR, Types.VARCHAR, Types.VARCHAR, Types.VARCHAR, Types.VARCHAR,
					Types.VARCHAR, Types.VARCHAR, Types.INTEGER, Types.INTEGER};
			saveOrUpdate(buildMap(CALLBACK_INSERT, args, type));
		}
	}

	public HistoryTask getFirstTask(String orderId) {
		StringBuffer sql = new StringBuffer(QUERY_HIST_TASK);
		sql.append(" where parent_task_id = ? and order_id = ? ");
		List<Object> paramList = new ArrayList<Object>();
		paramList.add("start");
		paramList.add(orderId);
		sql.append(" order by create_time desc limit 1");

		List<HistoryTask> historyTasks = queryList(HistoryTask.class, sql.toString(), paramList.toArray());
	...
	public HistoryTask getLastHistoryTaskByOrderId(String orderId) {
		StringBuffer sql = new StringBuffer(QUERY_HIST_TASK);
